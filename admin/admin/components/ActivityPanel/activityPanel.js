...
const findVolunteersForActivity = async (activity) => {
	...
	for (let i = 0; i < volunteers.length; i++) {
		if (areaSuitable(activity.areaNum, volunteers[i].areas)) {
			if (dateSuitable(activity.date, volunteers[i])) {
				if (
					professionFits(
						activity.activitieProfessions,
						volunteers[i].professions
					)
				) {
					console.log("suitable volunteer", volunteers[i]);
					suitableVolunteers.push(volunteers[i]);
				}
			}
		}
	}
	console.log("suitableVolunteers", suitableVolunteers);
};
